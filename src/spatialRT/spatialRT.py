...
class scDEC(object):
    ...
    def ramp_func(self, x, value1=0, value2=0.1):
        assert value1 < value2 
        slope = 1./(value2 - value1)
        return slope*(tf.nn.relu(x-value1) - tf.nn.relu(x-value2))

    @tf.function
    def train_gen_step(self, data_z, data_z_onehot, data_x, adj_hexigon):
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            data_z_combine = tf.concat([data_z, data_onehot_], axis=-1)
            data_x_ = self.g_net(data_z_combine)

            data_z_latent__, data_softmax__ = self.h_net(data_x_)
            data_z__ = data_z_latent__[:,:self.params['z_dim']]
            
            data_z_combine_ = tf.concat([data_z_, data_softmax_], axis=-1)
            ...
            ave_dis = tf.reduce_mean(distance_y)
            #smoothness loss
            ...
            g_h_loss = g_loss_adv+h_loss_adv+self.params['alpha']*(l2_loss_x + l2_loss_z)+ \
                        self.params['beta']*CE_loss_z + 0.05* smooth_loss #+ 0.01*entropy_loss
        ...
        return g_loss_adv, h_loss_adv, l2_loss_x, l2_loss_z, CE_loss_z, g_h_loss, smooth_loss, ave_dis
    ...
    def train(self):
        ...
        for batch_idx in range(self.params['nb_batches']):
            ...
            batch_x, batch_adj, batch_adj_hexigon, batch_x_neighbors, batch_adj_neighbors, batch_coor = self.x_sampler.get_batch(batch_size)            
            g_loss_adv, h_loss_adv, l2_loss_x, l2_loss_z, CE_loss_z, g_h_loss, smooth_loss, ave_dis = self.train_gen_step(batch_z, batch_z_onehot,batch_x,batch_adj_hexigon)
            if batch_idx % batches_per_eval == 0:
                ...
                print("Batch_idx [%d] g_loss_adv [%.4f] h_loss_adv [%.4f] smooth_loss [%.4f] ave_dis [%.4f] l2_loss_z [%.4f] l2_loss_x [%.4f] g_h_loss [%.4f] dz_loss [%.4f] dx_loss [%.4f] d_loss [%.4f]" % 
                      (batch_idx, g_loss_adv, h_loss_adv, smooth_loss, ave_dis, l2_loss_z, l2_loss_x, g_h_loss, dz_loss, dx_loss, d_loss))
